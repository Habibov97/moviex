...
- **The choreography lives in `globals.css` under `.mx-add-fab`, not in the component's class attribute** — the one exception in this app besides `.mx-range`. It needs a pseudo-element with its own `@keyframes` and three targets on three clocks; written as arbitrary variants it would be unreadable. The button's *appearance* is still Tailwind utilities and `--mx-*` tokens on the element. Three layers move, and the slight disagreement between them is what reads as alive rather than as one flat transform:
  ...
  2. **A sonar wave** on `::before`: `scale(1) / opacity 0.5` → `scale(2.3) / opacity 0`, **1300ms, `ease-out`, `infinite`**. Keyed to the **card's** hover, so it is already running as the button fades in.
  3. **The plus breathes** — `scale(1)` → `1.15` → `1`, `ease-in-out`, on the **same 1300ms period** so it is at its fullest as each wave leaves. Same card-hover trigger.
- **The two loops stop wherever they are when the pointer leaves**, and that is fine: the button is fading out over the top of it. The breathe keyframes start *and* end at rest so an interruption at either end is seamless, and nothing needs a clean exit frame.
...
- **The wave is a stroked ring, not a filled disc, and that is what makes the long throw possible.** The poster is `overflow-hidden` and the ring's centre sits 29px from its right and bottom edges, so **anything past a 29px radius is cut off** — the earlier filled version was capped at `scale(1.28)` for exactly that reason. A filled disc at `scale(2.3)` stops reading as a wave a third of the way out and becomes a red flood in the corner bounded by two hard straight edges. A stroke just has its arc leave the frame: the wave travels up and to the left across the poster and exits, which is what sonar from a corner actually looks like. **If it is ever made solid again, the ceiling goes back to ~1.28.**
  - The 2px stroke is scaled by the transform along with everything else, so the wave thickens as it fades. Intentional.
  - Effective radius runs to `20 × 2.3 × 1.12 = 51.5px` (the ring is a **child** of the button, so it inherits the button's hover scale). It stays inside the poster on the other two sides even on the narrowest card — 2-up at 320px puts the centre 107px from the left edge and 175px from the top.
