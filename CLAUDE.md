...
- **The hover microinteraction is `transform` and nothing else**: the button takes `scale-[1.1]`, the plus takes `rotate-90`. Both composite on the GPU, so neither costs a layout or a paint — which matters on a grid rendering twenty of these. **Don't animate the glow's spread, the blur radius or the diameter to get a similar effect**; they look alike and are far more expensive. (Tailwind v4 emits these as the individual `scale:` / `rotate:` properties, and its `transition-transform` covers `transform, translate, scale, rotate`, so they do transition — verified in the compiled CSS.)
  - **Two transforms means two elements.** A scale on the button and a rotate on the icon cannot share one property, so the icon carries its own. It reacts to hover on the **button** via a named `group/add`, not to the article's unnamed `group` — that one covers the whole card, and the plus should not spin because the cursor crossed the title.
  - **Nothing extra is needed to keep it static on touch.** Tailwind v4 compiles `hover:` inside `@media (hover: hover)`, so a tap never triggers either transform. Confirmed on the built stylesheet: both the scale and the group-hover rotate land inside that media block.
  - `focus-visible` mirrors both, so a keyboard user gets the same affordance.
