...
- **Always visible, at every input type.** The old bar was `opacity-0` until `group-hover`, which then needed an `[@media(hover:none)]:opacity-100` rule to exist at all on touch. A mark this small does not need to hide, so there is one appearance and hover only animates it. The poster's hover scrim stays — it now reads as the card's hover state and gives the glow something to sit against.
- **The hover choreography lives in `globals.css` under `.mx-add-fab`, not in the component's class attribute** — the one exception in this app besides `.mx-range`. It needs a pseudo-element with its own `@keyframes` and three targets on three different clocks; written as arbitrary variants it would be unreadable. The button's *appearance* is still Tailwind utilities and `--mx-*` tokens on the element. Three layers move at once, and the slight disagreement between them is what reads as textured rather than as one flat transform:
  1. **The button** springs to `scale(1.12)` over 280ms on `cubic-bezier(0.34, 1.56, 0.64, 1)` — an easeOutBack that overshoots ~10% past the target before settling.
  2. **A sonar ping** on `::before`: `scale(1) / opacity 0.5` → `scale(1.28) / opacity 0`, 560ms, decelerating. Single-shot per hover-in, never looping.
  3. **The plus** tilts `rotate(15deg) scale(1.15)` on the same spring, so it lifts *relative to* the button rather than merely riding its scale.
- **Only `transform` and `opacity` animate, on all three.** Both composite on the GPU, so nothing costs a layout or a paint — which matters on a grid drawing twenty of these. **Don't animate the glow's spread, the blur radius or the diameter** to get a similar effect: they look alike and are far more expensive.
- **`1.28` on the ring is a measured ceiling, not a taste call.** The poster is `overflow-hidden`, so a ping that grows too far is cut off mid-flight against the card's edge. The button is 40px at `bottom/right: 9px`, putting the ring's centre 29px from both edges; and the ring is a **child** of the button, so it inherits that `scale(1.12)` on top of its own. Effective radius is `20 × 1.28 × 1.12 = 28.7px` against 29px available. The obvious 1.6 would reach 35.8px and clip visibly. **Recompute it if the diameter, the offset or the hover scale ever change** — the arithmetic is in the CSS comment.
- **The ring paints above the button's background, not behind it, and that is forced rather than chosen.** CSS paints negative-`z-index` children *after* the parent's own background (Appendix E painting order), and this button establishes a stacking context anyway (`z-10` plus `backdrop-filter`), so a `z-index: -1` could not escape behind the disc — no arrangement of a `::before` can. It costs nothing visually because the ring is the **same hue** as the button: over the disc it is an imperceptible flush, and what reads is the halo outside it. The one thing it did cost is the icon, which would have been washed out by a 0.5-alpha pass; `.mx-add-fab-icon` is `position: relative; z-index: 1` to lift the plus above it.
...
- **The ping retriggers with no JS.** `:hover` adds the `animation` declaration and un-hovering removes it, which cancels it; re-entering re-adds it and it restarts from 0%. No animation-name toggling, no `requestAnimationFrame`, nothing to reset.
- **Nothing extra is needed to keep it static on touch.** The hand-written rules sit inside `@media (hover: hover)`, matching how Tailwind v4 wraps its own `hover:` variant, so a tap triggers none of it. `focus-visible` mirrors all three and sits *outside* that query, since it never fires on a tap and a keyboard user should get the same affordance.
- **Its colours are tokens, like everything else** — `--mx-add-fab` (`--mx-accent` at 92%, so the poster reads faintly through the glass), `--mx-add-fab-border` (`rgba(255,255,255,0.2)`), `--mx-add-fab-glow` (the `0 4px 14px` drop) and `--mx-add-fab-fg` (`#fafafa`, the plus). All four sit on `:root` with **no `.dark` override**, alongside `--mx-poster-*` and the tag colours: they sit on artwork, not on a page surface, so the theme does not change them.
  - **The plus was `--mx-on-accent` (`#2c0808`) first and is white now**, because the dark ink read as a smudge on the translucent disc. It got its **own** token rather than a new value for `--mx-on-accent`: that one is shared by every accent surface in the app, so repointing it would have turned the auth modal's submit, the row's Add button and the detail page's actions white too. It is also deliberately not `--mx-poster-fg`, despite the identical `#fafafa` — that token means "text laid directly on artwork", and this sits on the disc.
