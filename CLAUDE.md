...
- **It used to `router.replace(DISCOVER_HREF)` instead; don't reinstate that.** Silently landing someone on a different page with no explanation reads as a broken link, and it threw away the one moment where signing up has an obvious payoff.
- **The navbar's "My list" link is now gated too, and the two are not redundant — they cover different ways in.** A signed-out *click* on the link opens `LoginRegisterModal` on its login view instead of navigating; reaching `/my-list` any other way — typed, bookmarked, back/forward, middle-clicked, JS not yet hydrated — still lands on the page and meets `SignInRequired`. **The page guard is the protection; the navbar is the common-path UX.** Removing either leaves a real hole.
  - The gate is `useLibraryActions()`'s `requireAuth` / `authModal`, the same pair behind every Add / Mark-as-watched button — *not* a second auth check written for the navbar. So the three-way discipline is inherited rather than restated: only a **confirmed** `isSignedIn` lets the click through, and while `/auth/me` is in flight the link is inert rather than treated as signed-out (which would flash the modal at someone who is signed in).
  - It intercepts the click with `preventDefault()` and leaves the `href` alone. That is what keeps the anchor a real link — copyable, middle-clickable, and still correct with JS off.
  - **Which links are gated lives in `NAV_LINKS`** as `requiresAuth: true`, not as an `href === '/my-list'` test in the component — same reason the hrefs themselves live there. The mobile sheet closes on a gated tap only when something visibly happened (navigation or the modal); during the inert loading moment it stays open, since closing it would read as a dead link.
