...
- **`discover.markWatched` and `discover.markWatchedLabel` are now orphans** in all three message files, kept rather than deleted because the same wording lives on under `myList` and `detail` and this was a behaviour change, not a copy change. Delete them if the row's button is ever redesigned again.
...
`MovieCard`'s Add button is a **34px circle, bottom-right of the poster** (`right-[9px] bottom-[9px]`) — not the full-width accent bar it used to be along the bottom edge. The badges own the top of the poster (status left, rating right), so the bottom-right corner is the only spot that collides with neither, and a circle that small leaves the artwork readable.

- **Always visible, at every input type.** The old bar was `opacity-0` until `group-hover`, which then needed an `[@media(hover:none)]:opacity-100` rule to exist at all on touch. A mark this small does not need to hide, so there is one appearance and hover adds only a `scale-[1.08]` microinteraction. The poster's hover scrim stays — it now reads as the card's hover state and gives the glow something to sit against.
- **Its colours are tokens, like everything else** — `--mx-add-fab` (`--mx-accent` at 92%, so the poster reads faintly through the glass), `--mx-add-fab-border` (`rgba(255,255,255,0.2)`) and `--mx-add-fab-glow` (the `0 4px 14px` drop). All three sit on `:root` with **no `.dark` override**, alongside `--mx-poster-*` and the tag colours: they sit on artwork, not on a page surface, so the theme does not change them. The plus icon is `text-mx-on-accent` — the existing `#2c0808` dark ink every accent surface in the app already uses, not a new value.
