...
- Key parity is checkable: flatten all three files and diff the key sets. They must be identical — 214 keys as of writing.
...
## Movie list flow: what a card's one button does
...
1. A film the user has not saved has no state at all (`MovieSummary.userState` absent/`null`). Its action is **Add** (`discover.add`), which puts it on the watchlist.
2. A film on the watchlist is `userState: 'watchlist'` and shows the `discover.tagListed` tag. Its action is **Mark as watched** (`discover.markWatched`).
3. A watched film is `userState: 'watched'` and shows the `discover.tagWatched` tag. It keeps the **Add** action, because the watchlist and watched are separate lists — having seen a film does not put it back on the list.

Consequences to respect when touching the discover screens:

- **Every row/card always offers an action; no state renders an empty button slot.** Only `watchlist` swaps Add for Mark as watched.
- **`userState` is filled in client-side, not by the API.** `/tmdb/*` is public and never joins a user's list onto its results — `DiscoverSection` and `SearchResults` merge `useMovieStatuses(tmdbIds)` onto each `MovieSummary` before rendering. (The field's doc comment in `packages/shared-types/src/movie.ts` still describes an API-side join that was never built; the shape is right, the attribution is not.)
...
- `MovieUserState` in `packages/shared-types/src/movie.ts` is an **alias of `UserMovieStatus`** (`'watchlist' | 'watched'`), deliberately so cards, the detail page and the `user-movies` table speak one vocabulary. `userState` is optional because it is absent for signed-out users, so "absent" and "not in the list" are the same case in the UI.
...
- The state tag renders through `components/discover/StatusTag.tsx`, shared by the grid card and the list row so the label/colour mapping cannot drift between views (`STATUS_TAGS` is `satisfies Record<MovieUserState, …>`).
- The list row derives its button from `ROW_ACTIONS` in `components/discover/MovieRow.tsx`, a total map keyed by state (`satisfies Record<MovieUserState | 'none', RowAction>`, so a new state fails to compile until it has an action). Each entry holds message **keys**, not labels. Adding a state means adding an entry there, not a conditional at the call site. `useLibraryActions().runCardAction` mirrors that same branch, so the label and the effect cannot disagree.
- `userState` being a single enum cannot express "on the watchlist **and** watched". Today `'watched'` wins and the row still offers Add; if both need to show at once, that is a change to `UserMovieStatus`, not to the components.
